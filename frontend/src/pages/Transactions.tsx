...
export default function Transactions() {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (response.data.success) {
          // Transform API response (strings) into Category objects
          const incomeCategories: Category[] = response.data.data.income.map((name: string) => ({
            name,
            type: 'income' as const,
            icon: '💰', // Default icon for income
          }));

          const expenseCategories: Category[] = response.data.data.expense.map((name: string) => ({
            name,
            type: 'expense' as const,
            icon: '💸', // Default icon for expenses
          }));
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error?.message || 'Failed to delete transaction. Please try again.';
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {loading ? (
            ...
            pagination.totalCount === 0 ? (
              /* No transactions at all - New User */
              <EmptyState
                icon={
                  <svg
                    className="h-16 w-16"
